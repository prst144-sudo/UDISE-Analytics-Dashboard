...
import MCU_DATA from '../data/mcu_data.json';
...
export default function McuDashboard({ selectedYear, onShowPowerBi }) {
  const key = toKey(selectedYear);
  const { tab: tab, setTab: setTab, tabsRef, panelDir } = useTabSlider(['loc', 'grades']);
  useReveal(key);
  ...
  const fallback = Object.keys(MCU_DATA).sort().at(-1);
  const d = (key && MCU_DATA[key]) ? MCU_DATA[key] : MCU_DATA[fallback];
  if (!d) return null;

  const { header, cards, chartLoc, chartMgmt, chartLocMgmtSource, chartLocNote,
          chartGrades, chartGradesSource, chartGradesNote, insight1, insight2 } = d;

  return (
    <div className="dashboard"><section className="section socio-section">
      <div className="socio-header">
        <div>
          <div className="section-label" style={{color:'#6366f1'}}>{header.label}</div>
          <h2 className="section-headline">{header.headline}</h2>
          <p className="section-sub" style={{marginBottom:0}}>{header.sub}</p>
        </div>
        <button className="powerbi-btn" onClick={() => onShowPowerBi?.('https://app.powerbi.com/view?r=eyJrIjoiN2M1MzdkZjAtZGYyZC00YjgwLWFiYzMtYzE4MzUyN2EzZGYyIiwidCI6IjJiZDAyMzJmLWNjZTEtNDZkMS04ZWE2LTQzMTE1NTVlMzAyYiJ9')}>
      ...
      <div className="socio-bottom">
        <div className="socio-charts">
          <div className="socio-chart-tabs" ref={tabsRef}>
            <button className={`sct-tab ${tab==='loc'?'sct-active':''}`} onClick={()=>setTab('loc')}>MCU Share by Location + Management</button>
            <button className={`sct-tab ${tab==='grades'?'sct-active':''}`} onClick={()=>setTab('grades')}>Grade Combinations in MCU Schools</button>
          </div>

          {tab==='loc'&&<div key={tab} className="socio-chart-panel" data-dir={panelDir}>
            <div className="scp-source">{chartLocMgmtSource}</div>
            <div className="td-dual-charts" style={{marginTop:16}}>
              <div><div className="td-chart-subtitle">MCU Share by Location</div>
                <div className="states-list" style={{marginTop:10}}>{chartLoc.map(r=>(
                  <div key={r.name} className="state-row" style={{alignItems:'flex-start',paddingBottom:4}}>
                    <div className="state-name" style={{width:100,paddingTop:4}}>{r.name}</div>
              ...
              <div><div className="td-chart-subtitle">MCU Share by Management</div>
                <div className="states-list" style={{marginTop:10}}>{chartMgmt.map(r=>(
                  <div key={r.name} className="state-row" style={{alignItems:'flex-start',paddingBottom:4}}>
                    <div className="state-name" style={{width:124,paddingTop:4}}>{r.name}</div>
                    <div style={{flex:1}}><div className="state-track"><div className="state-bar" data-bar-w={r.pct} style={{width:0,background:r.color}}/></div>{r.note&&<div style={{fontSize:9,color:'var(--muted)',marginTop:2}}>{r.note}</div>}</div>
                    <div className="state-val" style={{fontFamily:"'DM Sans', sans-serif",fontSize:14,fontWeight:800,color:r.color,width:44}}>{r.val}</div>
                  </div>
                ))}</div>
          ...
          {tab==='grades'&&<div key={tab} className="socio-chart-panel" data-dir={panelDir}>
            <div className="scp-source">{chartGradesSource}</div>
            <div className="states-list" style={{marginTop:16}}>{chartGrades.map(r=>(
              <div key={r.name} className="state-row" style={{alignItems:'flex-start',paddingBottom:4}}>
                <div className="state-name" style={{width:228,paddingTop:4}}>{r.name}</div>
                <div style={{flex:1}}><div className="state-track"><div className="state-bar" data-bar-w={r.pct} style={{width:0,background:r.color}}/></div>{r.note&&<div style={{fontSize:9,color:'var(--muted)',marginTop:2}}>{r.note}</div>}</div>
                <div className="state-val" style={{fontFamily:"'DM Sans', sans-serif",fontSize:14,fontWeight:800,color:r.color,width:44}}>{r.val}</div>
              </div>
            ))}</div>
            <div className="scp-note">{chartGradesNote}</div>
          </div>}
        </div>
