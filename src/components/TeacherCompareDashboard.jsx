...
export default function TeacherCompareDashboard({ selectedYear, onShowPowerBi }) {
  const key = toKey(selectedYear);
  const { tab: tab, setTab: setTab, tabsRef, panelDir } = useTabSlider(['gender', 'deficit']);
  useReveal(key);
  ...
  if (!yr) return null;
  const { header, cards, chartGenderCat, chartGenderCatSource, chartGenderCatNote,
          chartDeficitMgmt, chartDeficitMgmtSource, chartDeficitMgmtNote,
          insight1, insight2 } = yr.teacherCompare;
  const CAT = '#0891b2';
  return (
    <div className="dashboard"><section className="section socio-section">
      <div className="socio-header">
        ...
        <button className="powerbi-btn" onClick={() => onShowPowerBi?.('https://app.powerbi.com/view?r=eyJrIjoiZGYwYmE0NzItNDMyMC00NGU3LTgwYjAtZWRlNjRjYWI4ZWIwIiwidCI6IjJiZDAyMzJmLWNjZTEtNDZkMS04ZWE2LTQzMTE1NTVlMzAyYiJ9')}>
      ...
      <div className="socio-bottom">
        <div className="socio-charts">
          <div className="socio-chart-tabs" ref={tabsRef}>
            <button className={`sct-tab ${tab==='gender'?'sct-active':''}`} onClick={()=>setTab('gender')}>Female Teacher Share by School Category</button>
            <button className={`sct-tab ${tab==='deficit'?'sct-active':''}`} onClick={()=>setTab('deficit')}>Teacher Deficit by Management Type</button>
          </div>
          {tab==='gender'&&<div key={tab} className="socio-chart-panel" data-dir={panelDir}>
            <div className="scp-source">{chartGenderCatSource}</div>
            <div className="states-list" style={{marginTop:16}}>{chartGenderCat.map(r=>(
              <div key={r.name} className="state-row">
                <div className="state-name" style={{width:220}}>{r.name}</div>
                <div className="state-track"><div className="state-bar" data-bar-w={r.pct} style={{width:0,background:r.color}}/></div>
                <div className="state-val" style={{fontFamily:"'DM Sans', sans-serif",fontSize:15,fontWeight:800,color:r.color,width:48}}>{r.val}</div>
              </div>
            ))}</div>
            <div className="scp-note">{chartGenderCatNote}</div>
          </div>}
          {tab==='deficit'&&<div key={tab} className="socio-chart-panel" data-dir={panelDir}>
            <div className="scp-source">{chartDeficitMgmtSource}</div>
            <div className="states-list" style={{marginTop:16}}>{chartDeficitMgmt.map(r=>(
