...
import CWSN_T_DATA from '../data/cwsn_teacher_data.json';

/*
 * SOURCE: cwsn_teacher_overall
...
export default function CwsnTeacherDashboard({ selectedYear, onShowPowerBi }) {
  const key = toKey(selectedYear);
  const { tab: tab, setTab: setTab, tabsRef, panelDir } = useTabSlider(['mgmt', 'dis']);
  useReveal(key);
  ...
  const fallback = Object.keys(CWSN_T_DATA).sort().at(-1);
  const d = (key && CWSN_T_DATA[key]) ? CWSN_T_DATA[key] : CWSN_T_DATA[fallback];
  if (!d) return null;

  const { header, cards, chartMgmtTraining, chartMgmtSource, chartMgmtNote,
          chartDisability, chartDisabilitySource, chartDisabilityNote,
          insight1, insight2 } = d;
  ...
  return (
    <div className="dashboard"><section className="section socio-section">
      <div className="socio-header">
        ...
        <button className="powerbi-btn" onClick={() => onShowPowerBi?.('https://app.powerbi.com/view?r=eyJrIjoiZmZlYWNjZGMtYThlMi00NjBlLWJkZTgtNDdhYzJiNjAyMDkxIiwidCI6IjJiZDAyMzJmLWNjZTEtNDZkMS04ZWE2LTQzMTE1NTVlMzAyYiJ9')}>
      ...
      <div className="socio-bottom">
        <div className="socio-charts">
          <div className="socio-chart-tabs" ref={tabsRef}>
            <button className={`sct-tab ${tab==='mgmt'?'sct-active':''}`} onClick={()=>setTab('mgmt')}>CWSN Training Rate by Management</button>
            <button className={`sct-tab ${tab==='dis'?'sct-active':''}`} onClick={()=>setTab('dis')}>Teacher Disability Type Distribution</button>
          </div>
