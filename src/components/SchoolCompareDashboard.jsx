...
export default function SchoolCompareDashboard({ selectedYear, onShowPowerBi }) {
  const key = toKey(selectedYear);
  const { tab: tab, setTab: setTab, tabsRef, panelDir } = useTabSlider(['zero', 'band']);
  useReveal(key);
  ...
  if (!yr) return null;
  const { header, cards, chartZeroMgmt, chartZeroMgmtSource, chartZeroMgmtNote,
          chartSingleBand, chartSingleBandSource, chartSingleBandNote,
          insight1, insight2 } = yr.schoolCompare;
  const CAT = '#6366f1';
  return (
    <div className="dashboard"><section className="section socio-section">
      <div className="socio-header">
        ...
        <button className="powerbi-btn" onClick={() => onShowPowerBi?.('https://app.powerbi.com/view?r=eyJrIjoiY2NkNGQ0NzItZjE4Yy00YjdjLTk1ODgtZDNhZjQ3OTUzOGE4IiwidCI6IjJiZDAyMzJmLWNjZTEtNDZkMS04ZWE2LTQzMTE1NTVlMzAyYiJ9')}>
      ...
      <div className="socio-bottom">
        <div className="socio-charts">
          <div className="socio-chart-tabs" ref={tabsRef}>
            <button className={`sct-tab ${tab==='zero'?'sct-active':''}`} onClick={()=>setTab('zero')}>Zero-Teacher Schools by Management</button>
            <button className={`sct-tab ${tab==='band'?'sct-active':''}`} onClick={()=>setTab('band')}>Single-Teacher Schools by Enrollment Band</button>
          </div>
          {tab==='zero'&&<div key={tab} className="socio-chart-panel" data-dir={panelDir}>
            <div className="scp-source">{chartZeroMgmtSource}</div>
            <div className="states-list" style={{marginTop:16}}>{chartZeroMgmt.map(r=>(
              <div key={r.name} className="state-row" style={{alignItems:'flex-start',paddingBottom:4}}>
                <div className="state-name" style={{width:168,paddingTop:4}}>{r.name}</div>
            ...
            <div className="scp-note">{chartZeroMgmtNote}</div>
          </div>}
          {tab==='band'&&<div key={tab} className="socio-chart-panel" data-dir={panelDir}>
            <div className="scp-source">{chartSingleBandSource}</div>
            <div className="states-list" style={{marginTop:16}}>{chartSingleBand.map(r=>(
              <div key={r.name} className="state-row" style={{alignItems:'flex-start',paddingBottom:4}}>
            ...
            <div className="scp-note">{chartSingleBandNote}</div>
          </div>}
        </div>
