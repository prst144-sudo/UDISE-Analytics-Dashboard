...
import TRANSITION_DATA from '../data/transition_data.json';

/*
 * SOURCE: Drop_Transition_NEP_File.csv
 * T_Boys, T_Girls, T_Total = Transition rates · Class = education stage
...
export default function TransitionDashboard({ selectedYear, onShowPowerBi }) {
  const key = toKey(selectedYear);
  const { tab: tab, setTab: setTab, tabsRef, panelDir } = useTabSlider(['level', 'soc']);
  useReveal(key, tab);

  const fallback = Object.keys(TRANSITION_DATA).sort().at(-1);
  const d = (key && TRANSITION_DATA[key]) ? TRANSITION_DATA[key] : TRANSITION_DATA[fallback];
  if (!d) return null;
          ...
          <div className="socio-charts">
            ...
            {tab === 'level' && (
              <div key={tab} className="socio-chart-panel" data-dir={panelDir}>
                <div className="scp-source">{chartTransClassSource}</div>
                ...
                <div className="scp-note">{chartTransClassNote}</div>
              </div>
            )}
            ...
            {tab === 'soc' && (
              <div key={tab} className="socio-chart-panel" data-dir={panelDir}>
                ...
                <div className="scp-note">{chartTransSocNote}</div>
              </div>
            )}
          </div>
